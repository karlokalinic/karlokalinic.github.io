...
Research baseline: Survival Log (Midnight Workshop, 2026) is useful because its strongest design is not the zombie theme. It combines a fixed preparation window with live stockpile visibility, inventory logistics, an expandable home, cooking/farming/power, needs and morale, weather/blackouts, rescue/trade, background-dependent starts and repeated runs. Its post-launch updates also show where friction matters: crisis countdowns, cancellable actions, quick transfers, cross-container resource search, gradual cold-wave escalation and phone/message controls.

We copy none of its content. We extract interaction problems worth solving.

## P0 — build these before content production

### 1. GameClock + CrisisTimeline — EASY
...
Test: replay same seed and verify identical event timestamps.
...
### 5. ActionTask — EASY
Every action has duration, stamina cost, interruptibility and completion effect.
...
### 6. PhoneFeed / InformationSource — EASY–MEDIUM
Messages have source, confidence, agenda, timestamp and whether another character believes them.
...
### 7. SocialLedger — MEDIUM / CORE FEATURE
...
### 8. RunLog — EASY / MANDATORY
Every meaningful state transition becomes structured data: timestamp, actor, action, object, consequence, social tags.
...
Background changes starting access, knowledge and relationships, not arbitrary RPG +10 stats.
Examples: student, warehouse worker, nurse, unemployed tenant, municipal clerk.
Question: who begins a disaster already holding the keys?
